...
После одобрения администратором группы отзыва пользователя, отзыв публикуется в канале. Ссылку на канал указывается в
`.env` файле, например `CHANNEL_ID=@tehnik_shanel`.
После публикации, сообщение в канале публикуется со стороны канала. Если нужно указать автора, то можно это сделать в
настройках канала.
